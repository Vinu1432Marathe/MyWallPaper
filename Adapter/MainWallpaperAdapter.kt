...
class MainWallpaperAdapter(
    ...
    private var list: MutableList<ModelWallpaper>
) : RecyclerView.Adapter<MainWallpaperAdapter.VH>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.adpter_wallpaper, parent, false)
        return VH(view)
    }
...
//        Glide.with(context).load(urlFirst+item.albumImage+".webp").into(holder.imgWallpaper)
    ...
    fun updateList(newList: List<ModelWallpaper>) {
        val limitedNewList = newList.take(10)

        // Avoid adding duplicates (based on imageID)
        val existingIds = list.map { it.imageID }.toHashSet()
        val itemsToAdd = limitedNewList.filter { it.imageID !in existingIds }
